...
typedef enum {
	...
	SWITCH_JSON_API_INTERFACE,				//JSON接口
	SWITCH_DATABASE_INTERFACE,				//数据库接口
...
/*anno@suy:2020-10-5*/
typedef enum {
	SWITCH_STATUS_SUCCESS,				//成功 (大多数函数的公共返回值)
	SWITCH_STATUS_FALSE,				//失败
	SWITCH_STATUS_TIMEOUT,				//超时
	SWITCH_STATUS_RESTART,				//重新开始以前的操作
	SWITCH_STATUS_INTR,
	SWITCH_STATUS_NOTIMPL,				//请求的资源未响应
	SWITCH_STATUS_MEMERR,				//内存错误
	SWITCH_STATUS_NOOP,					//没有东西
	SWITCH_STATUS_RESAMPLE,				//重采样
	SWITCH_STATUS_GENERR,				//常规错误
	SWITCH_STATUS_INUSE,				//请求的资源正在使用中
	SWITCH_STATUS_BREAK,				//非致命中断
	SWITCH_STATUS_SOCKERR,				//socket错误
	SWITCH_STATUS_MORE_DATA,			//需要更多地数据
	SWITCH_STATUS_NOTFOUND,				//未找到
	SWITCH_STATUS_UNLOAD,				//未加载
	SWITCH_STATUS_NOUNLOAD,				//从不卸载
	SWITCH_STATUS_IGNORE,
	SWITCH_STATUS_TOO_SMALL,
	SWITCH_STATUS_FOUND,
	SWITCH_STATUS_CONTINUE,
	SWITCH_STATUS_TERM,					//终止
...
/*anno@suy:2020-10-5*/
typedef enum {
	SWITCH_CHANNEL_ID_LOG,				//写入当前定义的日志
	SWITCH_CHANNEL_ID_LOG_CLEAN,		//写入当前定义的日志，没有额外的文件/行/日期信息
	SWITCH_CHANNEL_ID_EVENT,			//以日志事件的形式写入事件引擎
